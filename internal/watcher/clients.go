...
func (w *Watcher) reloadClients(rescanAuth bool, affectedOAuthProviders []string, forceAuthRefresh bool) {
	...
	geminiAPIKeyCount, vertexCompatAPIKeyCount, claudeAPIKeyCount, codexAPIKeyCount, openAICompatCount, aicoAPIKeyCount := BuildAPIKeyClients(cfg)
	totalAPIKeyClients := geminiAPIKeyCount + vertexCompatAPIKeyCount + claudeAPIKeyCount + codexAPIKeyCount + openAICompatCount + aicoAPIKeyCount
	...
	totalNewClients := authFileCount + geminiAPIKeyCount + vertexCompatAPIKeyCount + claudeAPIKeyCount + codexAPIKeyCount + openAICompatCount + aicoAPIKeyCount
	...
	log.Infof("full client load complete - %d clients (%d auth files + %d Gemini API keys + %d Vertex API keys + %d Claude API keys + %d Codex keys + %d OpenAI-compat + %d AICO keys)",
		totalNewClients,
		authFileCount,
		geminiAPIKeyCount,
...
func BuildAPIKeyClients(cfg *config.Config) (int, int, int, int, int, int) {
	...
	if len(cfg.AICOKey) > 0 {
		aicoAPIKeyCount += len(cfg.AICOKey)
	}
	return geminiAPIKeyCount, vertexCompatAPIKeyCount, claudeAPIKeyCount, codexAPIKeyCount, openAICompatCount, aicoAPIKeyCount
}
